...
Handles: filename standardization, FFmpeg MP4 export, full-file orchestration.

PNG generation is delegated to png_generator.py.
"""
...
def standardize_filename(file_path: str) -> str:
    """
    Rename file if it contains special characters that break FFmpeg/shell/NTFS.
...
def get_audio_duration(file_path: str):
    ...
    try:
        ...
        if ext == '.amr':
            duration = len(AudioSegment.from_file(file_path, format="amr")) / 1000
        elif ext in ['.m4a', '.3gp', '.3ga']:
            duration = MP4(file_path).info.length
        elif ext == '.wav':
            with wave.open(file_path, 'rb') as wf:
                duration = wf.getnframes() / float(wf.getframerate())
        elif ext == '.mid':
            duration = mido.MidiFile(file_path).length
        else:
            duration = MutagenFile(file_path).info.length
...
def process_file(file_path: str, output_dir: str, png_tmp_dir: str,
    ...
    Returns dict: {status: 'done'|'skipped'|'error', output_mp4, error}
    """
    from png_generator import generate_png
    ...
    try:
        ...
        png_path = generate_png(
            file_path, png_tmp_dir, style, bg_color,
            get_audio_duration_fn=get_audio_duration
        )

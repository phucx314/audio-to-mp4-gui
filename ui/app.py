...
class _BaseApp:
    ...
    def _init_app(self):
        ...
        try:
            ...
            _icon_path = Path(__file__).parent.parent / "assets" / "app-icon" / "mp3-to-mp4.png"
            _icon = PhotoImage(file=str(_icon_path))
            self.iconphoto(True, _icon)
            self._app_icon = _icon  # keep reference to prevent GC
        except Exception:
            pass
        ...
        self._mode              = ctk.StringVar(value="Audio \u2192 MP4")
    ...
    def _build_settings(self, parent):
        ...
        self._sec(parent, "Operation Mode", r); r += 1
        ctk.CTkSegmentedButton(
            parent, values=["Audio \u2192 MP4", "Hide File"],
            variable=self._mode, font=("Inter", 12),
    ...
    def _pick_files(self):
        ft = [("Audio files", "*.mp3 *.m4a *.aac *.wav *.amr *.mid *.3ga *.3gp *.wma *.awb *.ogg *.flac"), ("All files", "*.*")]
        if self._mode.get() == "Hide File":
            ft = [("All files", "*.*")]
            
        for f in filedialog.askopenfilenames(title="Select files", filetypes=ft):
            self._add_file(f)
